...
async def get_calendar(obra_id: int, db: DbSession, _: CurrentUserId) -> WorkingCalendarRead:
...
async def update_calendar(
    obra_id: int, data: WorkingCalendarUpdate, db: DbSession, _: CurrentUserId
) -> WorkingCalendarRead:
...
async def add_exception(
    obra_id: int, body: CalendarExceptionCreate, db: DbSession, _: CurrentUserId
) -> CalendarExceptionRead:
...
async def delete_exception(
    obra_id: int, exception_id: int, db: DbSession, _: CurrentUserId
) -> None:
...
async def load_holidays(
    obra_id: int, db: DbSession, _: CurrentUserId, year: int = Query(2026)
) -> dict:
