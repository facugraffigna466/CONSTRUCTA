...
import {
  ...
  fetchBitacora, fetchBitacoraUnassigned, reprocessEntry, setTranscript,
  type BitacoraEntry,
} from "../api/bitacora";
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {entry.suggestions!.map(s => (
                <SuggestionCard
                  key={s.id}
                  s={s}
                  onResolved={updated => onUpdated({
                    ...entry,
                    suggestions: (entry.suggestions ?? []).map(x => (x.id === updated.id ? updated : x)),
                  })}
                />
              ))}
            </div>
