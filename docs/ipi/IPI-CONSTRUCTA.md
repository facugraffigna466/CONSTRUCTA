...
| 6 | Sistema de alertas | Implementado, ampliado a detección proactiva de riesgo | Servicio de evaluación de riesgos (reactivo al abrir una obra y periódico para obras sin tráfico), **17 tipos de alerta** con severidad graduada, emisión en tiempo real por Socket.IO y traza en el historial. A las 6 reglas iniciales —que solo miraban fechas de vencimiento y respuestas del asistente— se sumaron 11 reglas de detección de riesgo que explotan datos que el sistema ya calculaba sin usarlos para alertar: ruta crítica, línea base, aprovisionamiento de materiales, calendario laboral e historial de eventos. Cada regla es configurable por empresa (interruptor y umbral) y sus alertas se resuelven automáticamente cuando la condición que las originó deja de detectarse, de modo que la reaparición del problema vuelva a avisar. Las de severidad crítica se envían además por WhatsApp al responsable de la obra, respetando su horario laboral. Auditoría dedicada en `docs/auditoria/06-alertas.md`, cerrada el 26/08/2026; diseño de las reglas en `docs/estado/propuesta-reglas-riesgo.md` e implementación documentada en `docs/features/deteccion-riesgos.md` (03/09/2026). | Ninguno relevante para el objetivo comprometido. |
...
| 8 | Panel de indicadores | Implementado | Panel de obra con dieciséis indicadores: avance real ponderado por duración, avance planificado, índice de desempeño del cronograma (SPI), fecha de fin proyectada, salud de la ruta crítica, desvío contra la línea base, estado de hitos, ejecución de materiales, alertas por severidad, cuello de botella y una curva de evolución planificado/real historizada día a día por un job del planificador, servidos en una sola llamada (`GET /obras/{id}/dashboard`); más un acordeón mensual con concentración de retrasos, precisión de estimación por disciplina, evidencia de las tareas con mayor desvío (bitácora, alertas e impacto en cascada, sin identificar al responsable), temas recurrentes de bitácora con su correlación con retrasos posteriores y velocidad de reacción a alertas por tipo, leídos de `GET /obras/{id}/dashboard/monthly-insights`. Cada bloque declara si el dato está disponible y por qué no lo está, en vez de completar con ceros. Diseño documentado en `docs/features/dashboard-indicadores-obra.md` (09/09/2026, extendido el 10/09/2026) e implementación en cuatro etapas incrementales, la última cerrada el 10/09/2026. | El toggle semana/mes de la curva de avance quedó acotado a semanal; el detalle por responsable de la concentración de retrasos se restringe a administradores por diseño, no por limitación técnica. |
...
| 10 | Documentación técnica centralizada | Implementado, con una limitación de la plataforma | Repositorio de planos con versionado por obra y disciplina; consulta y envío de la última versión vigente por WhatsApp; control de acceso por responsable y por disciplina. Auditoría dedicada en `docs/auditoria/05-planos.md`, cerrada el 21/08/2026, con una segunda ronda de correcciones surgidas de la prueba en producción (28/08/2026). | WhatsApp no admite adjuntos de más de 16 MB: un plano que supere ese tamaño se advierte en la interfaz al cargarlo y, si alguien lo solicita desde el campo, el asistente responde explicando por qué no puede enviarlo y a quién recurrir. La compresión automática que permitiría entregarlo igualmente quedó fuera de alcance. |
...
| 12 | Bitácora de obra con IA | Implementado | Cadena audio → transcripción (`gpt-4o-mini-transcribe`) → análisis (Claude Haiku 4.5, salida estructurada) → resumen, puntos clave y sugerencias aplicables. Las sugerencias son una entidad propia del dominio (tabla `suggestions`, migración 0072), no un objeto anidado en la nota: se consultan y se resuelven por identificador desde cualquier pantalla. Navegación completa en las dos direcciones —desde una tarea se ven las notas que la modificaron y las propuestas pendientes sobre ella, y se aplican sin salir de la tarea—. Auditoría dedicada en `docs/auditoria/08-bitacora.md` (aislamiento cross-tenant, cuota de IA por WhatsApp, límite de notas por hora, actualización en tiempo real, paginación), cerrada el 27/08/2026. | Ninguno relevante. La sugerencia sigue mostrando el nombre de la tarea como texto plano dentro de la bitácora, pero la navegación que esa brecha bloqueaba quedó resuelta por el otro extremo: la propuesta aparece dentro de la tarea. |

Como respaldo transversal a varios de estos objetivos, la suite automatizada del backend alcanza 619 tests distribuidos en 55 archivos (ejecutados en integración continua), que cubren aislamiento por tenant, autenticación, rate limiting e importaciones, sumados a 49 pruebas de frontend sobre la lógica de alertas; no existen todavía pruebas de extremo a extremo (véase Pruebas).
...
- **Frontend — aplicación de página única:** se utilizó React (React, 2026) con TypeScript (TypeScript, 2026) y Vite —con *Vitest* como marco de pruebas, que comparte su configuración y su cadena de transformación—, por su madurez, su tipado estático y la velocidad de su entorno de desarrollo. Para la planilla de tareas se incorporó además la biblioteca `react-datasheet-grid` (licencia MIT): reimplementar el comportamiento de una grilla editable —selección de rangos, relleno por arrastre, portapapeles, deshacer— demostró ser costoso y frágil en una primera versión propia, y delegarlo en una biblioteca específica permitió concentrar el esfuerzo en la lógica del dominio.
...
**Detección de Riesgo**
- **RF-73:** El sistema debe detectar automáticamente situaciones de riesgo cruzando la ruta crítica, la línea base, el estado de aprovisionamiento de materiales, el calendario laboral y el historial de la obra, sin requerir intervención del usuario.
- **RF-74:** El sistema debe anticipar el riesgo antes de que se materialice el atraso, avisando de un vencimiento en la ruta crítica, de un hito comprometido o de un material faltante **antes** de la fecha correspondiente.
- **RF-75:** El sistema debe asignar un nivel de severidad a cada alerta (crítica, alta, media o baja) y priorizar visualmente las de mayor severidad.
- **RF-76:** El sistema debe permitir a cada organización habilitar o deshabilitar individualmente cada regla de detección de riesgo y configurar su umbral de disparo.

**Dashboard de Indicadores**
- **RF-77:** El sistema debe calcular el avance real de una obra ponderado por la duración planificada de cada tarea (días laborables), de modo que una tarea de veinte días pese veinte veces más que una de un día.
- **RF-78:** El sistema debe calcular el avance planificado a la fecha, el índice de desempeño del cronograma (SPI) resultante de comparar ambos, y proyectar la fecha de fin de obra a partir del ritmo de avance vigente.
- **RF-79:** El sistema debe agregar en el panel de obra la salud de la ruta crítica (cantidad de tareas críticas, en riesgo y con holgura, y la cobertura del cálculo) y el desvío del cronograma actual respecto de la línea base guardada.
- **RF-80:** El sistema debe mostrar el estado de cada hito de la obra (cumplido, cumplido con demora, en riesgo o pendiente) en el panel.
- **RF-81:** El sistema debe mostrar la ejecución de materiales de la obra (porcentaje comprometido y recibido respecto del estimado) y su alineación con el avance real.
- **RF-82:** El sistema debe historizar diariamente el avance de cada obra activa y presentar su evolución planificada frente a la real en un gráfico temporal, además de exponer en el panel la concentración de retrasos por tarea y por responsable y la precisión histórica de estimación por disciplina, restringiendo el detalle por responsable a usuarios administradores.
- **RF-83:** El sistema debe exponer en el panel, para las tareas con mayor desvío del período, un paquete de evidencia (menciones de bitácora, alertas asociadas e impacto en cascada) sin identificar al responsable de la tarea; debe además agrupar las menciones de bitácora por tipo de problema recurrente con su correlación con retrasos posteriores, y mostrar la velocidad de reacción a alertas por tipo.

#### Requerimientos no funcionales

- **RNF-01 (Seguridad):** la API autentica las operaciones con tokens JWT, aplica permisos por rol (administrador/colaborador) y contraseñas con *hashing*; los recursos sensibles usan URLs firmadas. La auditoría del 13/08/2026 había identificado dos rutas sin filtro de tenant (presencia y simulación de vencidos administrativa); ambas quedaron corregidas en la remediación de las auditorías 06 y 11 (26/08/2026). Una verificación manual del 08/09/2026, al probar en el navegador un ajuste no relacionado del módulo de Compras, encontró un tercer punto sin filtrar: el listado, edición y borrado de proveedores (`/suppliers`) devolvía y permitía modificar registros de cualquier empresa, y la alta automática de proveedor al confirmar un contratista directo tampoco fijaba el tenant. Corregido el mismo día, con 4 pruebas de regresión nuevas en `test_tenant_isolation.py`.
...
- **RNF-06 (Mantenibilidad):** separación en capas (router → service → repository), esquema de base de datos versionado con migraciones (Alembic) y tipado estático en backend (Python con anotaciones de tipo) y frontend (TypeScript). El linter del frontend (ESLint, con las reglas de React 19 y de *Fast Refresh* de Vite) corre limpio de forma sostenida: el 10/09/2026 se resolvieron los 70 problemas preexistentes detectados en una revisión integral (no limitada a los módulos en desarrollo), separando en archivos propios los *hooks* y objetos de contexto que antes compartían archivo con un componente, y corrigiendo un desvío real de datos detectado al aplicar el fix (`GanttTimeline.tsx`: una referencia leída durante el renderizado del arrastre de filas del cronograma podía quedar desactualizada).
...
El modelo de datos, versionado en setenta y una migraciones (`0001` a `0071`), se organiza en torno a la entidad **obra** como agregado central y se compone de las siguientes entidades principales:
...
- **Comunicación y campo:** `messages` y `conversation_sessions` (estado del asistente de WhatsApp), `bitacora_entries` (notas de voz con su transcripción y resumen) y `suggestions` (las acciones propuestas por la IA sobre el plan, con su origen y su estado de resolución), `historial` (registro de eventos *append-only*), `alerts` (con su nivel de severidad).

El asistente de WhatsApp opera con dos canales de forma deliberadamente distinta según quién escribe. El personal administrativo envía **notas de voz** que alimentan la bitácora con IA: recorre la obra, registra lo relevante sin estructura y organiza después desde el escritorio. Los responsables de tarea operan con un **menú numérico**, que no exige redactar ni interpretar y se responde con una sola mano en condiciones de obra. Esa asimetría concentra además el costo y el margen de error del modelo de lenguaje en el único canal donde una persona revisa antes de que algo modifique el plan. Al informar una tarea bloqueada, el menú solicita también el **motivo** entre opciones predefinidas (falta de material, de personal, clima, dependencia de otra tarea): el bloqueo se registra primero y el motivo se incorpora después al historial y a la alerta, de modo que una conversación interrumpida no haga perder el reporte de campo.
- **Detección de riesgo:** `task_risk_snapshots` (holgura calculada en la corrida anterior, para detectar que se está achicando) y la configuración de reglas y umbrales por empresa, alojada en `settings`.
...
**Enfoque de desarrollo.** La construcción se abordó de forma **incremental**, con una rama de control de versiones (Git) por cada etapa del plan, integrada al tronco principal tras su verificación. Cada cambio en el modelo de datos se materializó en una migración versionada (setenta y cuatro en total, `0001` a `0074`), de modo que el esquema de la base pudiera evolucionar de manera reproducible. La cronología completa del desarrollo, con sus decisiones y validaciones, se documenta en `docs/documentacion.md`.
...
- **Módulo de planilla de tareas:** vista de edición tipo hoja de cálculo. Tras una primera versión de desarrollo propio que resultó poco fluida en la carga de datos, el módulo se reconstruyó sobre la biblioteca `react-datasheet-grid` (licencia MIT), de modo que el comportamiento genérico de una grilla —selección de celdas y rangos, navegación con teclado, relleno por arrastre, copiar y pegar bloques, y deshacer— quedara resuelto por una implementación ya probada, y el desarrollo propio se concentrara en las celdas específicas del dominio. La planilla reproduce las convenciones de una hoja de cálculo: **grilla que se extiende más allá de los datos** (una tarea se da de alta escribiendo directamente sobre cualquier celda vacía), **inserción de filas en cualquier posición del orden** (que se persiste), **columna de tarea fija** al desplazarse horizontalmente, **ancho de columna ajustable** por arrastre y **barra de estado** que muestra el resumen de la obra y, cuando hay varias celdas seleccionadas, el recuento, la suma y el promedio de sus valores numéricos. El usuario puede **mostrar u ocultar columnas** según su necesidad; además de las de planificación, dispone de columnas opcionales de **hito**, **predecesoras** —con la notación de Microsoft Project (`5FS+2` indica que la tarea comienza dos días después de finalizada la de la fila 5), acompañada de una ayuda desplegable en el encabezado— y **costo de materiales**, que despliega el detalle de materiales de la tarea y enlaza con el módulo de presupuesto. Se complementa con **importación** desde Excel, CSV y MS Project (mapeo de WBS, recursos y dependencias) y **exportación** a Excel, además de una plantilla descargable.
- **Módulo de alertas y detección de riesgo:** el sistema evalúa automáticamente el riesgo de cada obra y genera alertas de **diecisiete tipos**, cada una con un nivel de **severidad** (crítica, alta, media o baja) que ordena visualmente qué atender primero. Seis reglas iniciales cubren el estado inmediato —tarea bloqueada, riesgo de demora, tarea vencida, falta de respuesta, reprogramación solicitada y recepción de pedido—; a ellas se sumaron **once reglas de detección de riesgo** que cruzan información que el sistema ya calculaba pero no utilizaba para alertar: la **ruta crítica** (una tarea sin holgura que se atrasa corre la fecha de fin de toda la obra, y su holgura decreciente anticipa que está por volverse crítica), la **línea base** (desvío entre lo planificado y lo replanificado), el **aprovisionamiento** (materiales sin pedir, pedidos que el proveedor nunca confirmó y materiales que van a bloquear una tarea próxima a arrancar), el **calendario laboral** (vencimientos que caen en feriado o día no laborable), el **historial** (tareas que se bloquean una y otra vez, responsables que acumulan faltas de respuesta) y los **hitos** (compromisos próximos con tareas previas sin terminar). El cambio cualitativo es que el sistema deja de limitarse a informar que algo **ya** salió mal para avisar que **va a** salir mal, mientras todavía hay margen de reacción. Cada regla puede habilitarse o deshabilitarse por empresa y tiene su umbral configurable, y se ejecutan en tres frecuencias según su naturaleza: las que miran el estado del día cada cuatro horas, las que comparan contra el estado del día anterior una vez por día, y las que detectan patrones acumulados sobre el historial una vez por semana. Las alertas se emiten en tiempo real por Socket.IO, se deduplican para no repetir un aviso vigente y quedan trazadas en el historial. El mismo canal soporta la presencia de usuarios conectados y la edición colaborativa.
- **Módulo de asistente de WhatsApp:** un *webhook* recibe los mensajes entrantes; el sistema identifica al emisor (responsable de tareas o personal de la empresa) por su número y conduce la conversación mediante una **máquina de estados** (estados: reposo, selección de obra, selección de tarea, menú de estado y espera de fecha), persistida en la base. Permite reportar el estado de una tarea, registrar notas de voz y consultar planos. La resolución de identidad contempla el ciclo de vida del número: un responsable dado de baja deja de reservarlo (el número puede reasignarse al personal de la empresa, con un control que impide reactivar al responsable mientras otro lo tenga), y la comparación reconoce como equivalentes las dos formas en que se escribe un celular argentino (+54 y +549), que en la práctica conviven en los registros.
- **Módulo de bitácora de obra con IA:** convierte una nota de voz en información estructurada mediante una cadena de procesamiento: el audio se transcribe con un modelo de reconocimiento del habla (`gpt-4o-mini-transcribe`) y el texto resultante se analiza con un modelo de lenguaje (Claude Haiku 4.5) configurado con **salida estructurada (JSON Schema)**. El análisis produce un resumen de dos a cuatro oraciones, una lista de puntos clave y un conjunto de **sugerencias aplicables** sobre el plan —reprogramar una tarea, crear una tarea, cambiar un estado (con el **porcentaje de avance** si el audio lo menciona: «la mampostería va al 75%» se traslada al avance de la tarea, y nunca se inventa un porcentaje que el audio no dice), **reasignar el responsable** de una tarea existente a otro integrante del equipo de la obra, o dejar una nota—, cada una con una cita del audio que la justifica. Si el audio no contiene nada accionable, no se fuerzan sugerencias. El modelo recibe el **calendario laboral** de la obra para proponer fechas en días hábiles. Un hallazgo del uso real motivó una regla adicional: en la jerga de obra, 'correr la fecha para atrás', 'para adelante' y 'patearla' se emplean de forma contradictoria según el hablante, y una interpretación literal producía reprogramaciones en el sentido opuesto al pedido. La ambigüedad se resuelve por la **causa** y no por el verbo: si el motivo declarado es una demora (lluvia, material que no llegó, proveedor atrasado), la fecha solo puede desplazarse hacia adelante en el tiempo; si la dirección no queda determinada, el sistema registra una nota en lugar de proponer una reprogramación. Por la misma razón se le retiró al modelo el **cálculo de fechas**: contar días hábiles salteando fines de semana y feriados es una operación determinística sobre datos que el modelo no posee —los días no laborables propios de cada obra viven en la base—, y al hacerla producía desvíos de uno o dos días. El modelo declara ahora el corrimiento en días laborales y el backend calcula la fecha resultante contra el calendario de la obra; solo se acepta una fecha del modelo cuando el audio la nombra explícitamente. El resultado es, además, verificable por pruebas automatizadas, cosa que no era posible mientras la cuenta ocurría dentro de la llamada al modelo. Para reasignar el responsable de una tarea, el modelo recibe el **equipo de la obra** (no el directorio completo de la empresa, porque asignar una tarea exige que la persona ya esté en ese equipo) y matchea el nombre mencionado contra un id exacto de esa lista; si la persona nombrada no forma parte del equipo, no se propone la reasignación —queda una nota pidiendo sumarla primero desde la pestaña Responsables—, evitando una sugerencia que el sistema rechazaría al aplicarla. La cadena está endurecida contra su punto débil estructural —el audio se graba en la obra, con viento y ruido de máquinas, y un error de transcripción se propaga al análisis sin que el modelo de lenguaje pueda detectarlo, porque no escucha el audio— con dos medidas tomadas de la evidencia sobre cadenas ASR→LLM: la transcripción recibe un **glosario de jerga de obra** (unos cincuenta términos del rubro en Argentina) que sesga al reconocedor hacia el vocabulario técnico donde más se equivoca, y el análisis tiene instruido **marcar la incertidumbre en vez de corregir en silencio**: un fragmento que parece mal transcripto no habilita sugerencias de acción —se registra como nota citando el texto dudoso, para confirmarlo con el emisor— y la duda queda señalada en el resumen, de modo que el revisor humano pueda verla en lugar de heredar una interpretación fabricada. Las sugerencias se revisan y aplican (Sí/No) desde la aplicación —un módulo propio de cada obra— donde un indicador por obra señala las que quedan pendientes. Para no perder ninguna nota, si una nota de voz llega sin obra (un emisor con varias obras que no indicó cuál), el sistema le **recuerda automáticamente al emisor cada 30 minutos** —en horario laboral, hasta 48 horas— que la asigne; y si aun así no responde, la nota queda visible en una sección **"Sin asignar"** para que el jefe de obra la asigne manualmente, garantizando la trazabilidad. Antes de aplicar una sugerencia, el jefe puede **editarla** (ajustar fechas, título, responsable o estado): la IA propone y él decide. Cuando una sugerencia se aplica, el sistema **le confirma por WhatsApp a quien envió la nota** qué se hizo, cerrando el círculo del reporte; y la llegada de una nota nueva se anuncia al instante con una **notificación en tiempo real** (Socket.IO) al equipo de la obra. La trazabilidad tarea → nota está completa: desde una tarea se accede a las **notas de voz que la originaron o modificaron**, con el audio reproducible y la cita que lo justifica. El historial de notas se explora con **búsqueda y filtros** (por texto o responsable, por tipo de acción y por fecha).

- **Las sugerencias de IA como funcionalidad transversal:** las acciones que el modelo propone sobre el plan no pertenecen a la pantalla que las originó. Inicialmente se almacenaban como objetos anidados dentro del registro de la nota de voz y se resolvían por su posición en esa lista, lo que las ataba a un único módulo: no era posible consultarlas por tarea ni resolverlas desde otra pantalla. Se las promovió a **entidad del dominio** —tabla propia con identificador estable, estado (`pendiente`/`aplicada`/`descartada`) y registro de quién resolvió y cuándo— junto con un servicio dedicado que es el único responsable de aplicarlas, venga la orden de donde venga. El origen quedó modelado como un atributo (`source`) y no como propietario, de modo que una fuente futura —un mensaje de texto, un análisis de riesgo— no obliga a modificar a quien las consume. El efecto visible para el usuario es que **la propuesta aparece dentro de la tarea que quiere modificar**, con el contexto de su origen (quién la reportó y el resumen de la nota), y se aplica o se descarta ahí mismo, sin tener que recordar que existe el módulo de bitácora ni buscar la nota correspondiente.
...
- **Módulo de dashboard de indicadores:** reemplaza el panel de obra original —que contaba tareas completadas sin ponderar su tamaño— por dieciséis indicadores, diseñados primero como documento (`docs/features/dashboard-indicadores-obra.md`) y luego implementados en cuatro etapas incrementales. Los indicadores en vivo se sirven en una sola llamada (`GET /obras/{id}/dashboard`). El **avance real** se pondera por la duración planificada de cada tarea en días laborables, y se compara contra el **avance planificado** a la fecha para obtener el **índice de desempeño del cronograma (SPI)**, del que además se deriva una **fecha de fin proyectada**. Se agregan en el mismo panel la **salud de la ruta crítica** (tareas críticas, en riesgo y holgura mediana, con su porcentaje de cobertura), el **desvío contra la línea base** guardada, el **estado de cada hito** (cumplido, cumplido con demora, en riesgo o pendiente) y la **ejecución de materiales** frente al estimado, con un indicador de alineación entre lo recibido y el avance real. Cada bloque declara explícitamente si el dato está disponible y por qué no, en vez de mostrar un cero cuando no hay información suficiente —por ejemplo, sin línea base guardada el bloque correspondiente no se calcula, no se completa con ceros—. Para la **evolución histórica** (curva de avance planificado vs. real en el tiempo) se agregó una tabla nueva (`obra_progress_daily`) que un job diario del planificador completa hacia adelante, sin reconstruir el pasado desde el historial de eventos por ser una operación frágil; el ajuste no fue trivial, porque el punto semanal del gráfico y la fila guardada por el job rara vez caen en la misma fecha exacta —se emparejan por cercanía, con una tolerancia de tres días—.

  Un acordeón colapsado por defecto (`GET /obras/{id}/dashboard/monthly-insights`) expone cinco métricas que el motor de estadísticas mensual (`ObraStatsService`) ya calculaba para alimentar el informe interno con IA, pero que hasta el 10/09/2026 nunca habían llegado a una pantalla: la **concentración de retrasos** (regla 80/20, por tarea y por responsable; el detalle por responsable se filtra en el backend y solo se envía a usuarios administradores, para no convertir una herramienta de coordinación en una de evaluación de desempeño individual), la **precisión histórica de estimación** por disciplina, la **evidencia de las tareas con mayor desvío** (menciones de bitácora, alertas asociadas e impacto en cascada sobre tareas dependientes), los **temas recurrentes de bitácora** (falta de material, clima, ausencia de personal, proveedor, problema técnico, equipos y seguridad) con su correlación temporal con retrasos posteriores, y la **velocidad de reacción a alertas** por tipo. A diferencia de la concentración por responsable, estas tres últimas no traen ranking por persona y viajan igual para todos los roles; la de evidencia de desvíos sí omite deliberadamente el responsable de la tarea en lo que se muestra en pantalla —mismo criterio que la concentración por responsable, pero resuelto en el propio recorte de datos en vez de un filtro por rol—, porque es evidencia sobre una tarea y no un señalamiento de quién la causó. Para evitar que una correlación estadísticamente vacía se lea como un patrón, el porcentaje de correlación de un tema de bitácora solo se muestra a partir de tres menciones; por debajo, se muestra el conteo crudo.
...
- **Auditoría técnica sistemática:** se auditó el sistema módulo por módulo —las 25 rutas del backend, los servicios y el modelo de datos— con ocho análisis (`docs/analisis/analisis-modulo-*.md`) consolidados en un informe maestro (`docs/auditoria/auditoria-sistema-consolidada.md`) que clasifica los hallazgos por severidad (seguridad / robustez / pulido) e identifica como tema transversal principal el aislamiento entre empresas (*tenants*). Tras la defensa se realizó una segunda ronda sistemática, módulo por módulo, documentada en once informes (`docs/auditoria/01` a `11`), cuya remediación se completó entre el 21 y el 28 de agosto de 2026. La suite automatizada creció a 619 tests distribuidos en 55 archivos, ejecutados en integración continua ante cada cambio: aislamiento por tenant (`test_tenant_isolation.py`, que verifica que un usuario de una empresa no accede a los recursos de otra —incluida la fuga de proveedores cross-tenant detectada y corregida el 08/09/2026—), permisos por obra, autenticación, *rate limiting*, importaciones, robustez de infraestructura y un archivo por módulo auditado, entre ellos los cinco del dashboard de indicadores (`test_dashboard_calc.py`, `test_obra_dashboard.py`, `test_obra_progress_daily.py`, `test_curva_s.py`, `test_monthly_insights.py` — este último extendido el 10/09/2026 para cubrir la evidencia de desvíos, los temas de bitácora y la reacción a alertas del acordeón mensual, sin sumar archivos nuevos) y la resolución de identidad por número de WhatsApp (`test_liberar_numero_whatsapp.py`, incorporado el 11/09/2026 a raíz de un caso de uso real: un número dado de baja como responsable quedaba bloqueado para siempre y no podía reutilizarse para el personal de la empresa). A esa suite se sumaron 49 pruebas de frontend con *Vitest* sobre el módulo de alertas: el catálogo de tipos, los dos *hooks* que reciben los eventos en tiempo real y los dos componentes que muestran las alertas al usuario —el listado por obra y la campana del encabezado—, también en integración continua. La automatización de extremo a extremo queda pendiente y se declara en Objetivos no cumplidos.
- **Prueba en entorno real como fuente de hallazgos:** la verificación contra la integración real de mensajería (Twilio/WhatsApp), y no solamente la lectura de código o la suite automatizada, expuso defectos que ninguna de las otras técnicas había detectado: un menú de desambiguación que no aceptaba el nombre de la obra como respuesta y dejaba al usuario en un ciclo sin salida; un valor por defecto invertido en la asignación de permisos, latente porque ninguna interfaz ejercitaba ese camino; y un límite de tamaño de la plataforma de mensajería que el sistema no comunicaba. Se documenta como aprendizaje metodológico: en un sistema cuya interfaz principal es un canal de terceros, la prueba de integración real resulta una técnica de verificación no sustituible. El mismo principio se confirmó en otro plano al implementar la detección de riesgo: la suite automatizada corre sobre SQLite, y recién al aplicar las migraciones sobre PostgreSQL apareció una comparación entre una columna de tipo enumerado y un parámetro de texto que ese motor no admite, y que en SQLite pasaba inadvertida por representar los enumerados como texto.
...
| Riesgo en la ruta crítica | Encadenar dos tareas sin holgura y dejar vencer la primera | Se genera una alerta de severidad crítica que indica que la fecha de fin de obra ya se corrió, distinta de la alerta común de tarea vencida. |
| Material que va a bloquear una tarea | Cargar un material sin recibir en una tarea que arranca en pocos días | Se avisa antes de que la tarea llegue a estado Bloqueada, indicando qué material falta. |
| Configuración de una regla de riesgo | Deshabilitar una regla desde Configuración y ejecutar la evaluación | Esa regla deja de generar alertas y las demás siguen funcionando. |
| Avance ponderado sin línea base | Consultar el panel de una obra sin línea base guardada | El bloque de línea base indica que no está disponible y por qué, en vez de mostrar un desvío de 0 días. |
| SPI en cero sin avance | Consultar el panel de una obra con una tarea sin ningún avance pero con plan ya arrancado | El SPI se calcula en 0.0 (no como error) y la fecha de fin proyectada se oculta por no ser confiable, en vez de dividir por cero. |
| Curva de avance con tracking desalineado | Consultar la curva de una obra cuyo job diario arrancó a trackear en un día de la semana distinto al de inicio de la obra | Cada punto semanal del gráfico muestra el valor real más cercano dentro de una tolerancia de tres días, en vez de quedar vacío por no coincidir la fecha exacta. |
| Evidencia de desvíos por rol | Consultar el acordeón mensual con un usuario administrador y con uno colaborador | La evidencia de desvíos, los temas de bitácora y la reacción a alertas viajan igual para ambos roles; solo el ranking de concentración por responsable se omite para el colaborador. |
| Tema de bitácora con muestra chica | Una categoría de bitácora con una sola mención seguida de un retraso | El panel muestra el conteo crudo ("1 mención, 1 con retraso después") en vez de un porcentaje de correlación del 100%. |
| Análisis mensual por rol | Consultar el acordeón de análisis del período con un usuario administrador y con uno colaborador | El ranking de retraso por tarea se muestra a ambos; el ranking por responsable solo llega en la respuesta del administrador. |

> **Estado de la cobertura automatizada.** El backend cuenta con 619 tests automatizados (*pytest*, 55 archivos) y el frontend con 49 (*Vitest*, 5 archivos), todos corriendo en integración continua. La cobertura de frontend es deliberadamente parcial: cubre el módulo de alertas de punta a punta —de la lógica de presentación a los componentes que la usan—, por ser donde se concentraron los defectos detectados, y no alcanza todavía al resto de la interfaz. No existen pruebas de extremo a extremo (*Playwright*); se deja declarada esa brecha como parte de los objetivos pendientes del proyecto.
...
- **Lectura honesta del avance:** el panel de obra reemplaza el conteo simple de tareas completadas por un avance ponderado por duración, un índice de desempeño del cronograma (SPI) y una fecha de fin proyectada, de modo que una obra con tareas grandes sin terminar ya no aparece más avanzada de lo que está.
...
Quedan pendientes dos tipos de aspectos, de naturaleza distinta. Por un lado, dos funcionalidades declaradas explícitamente fuera de alcance del proyecto: la notificación automática al jefe o administrador de obra cuando un responsable responde a un recordatorio (funcionalidad analizada y diseñada, no implementada) y la pasarela de pago para el cobro efectivo de los planes de suscripción. Por otro lado, una brecha real detectada durante el desarrollo, hoy parcialmente cerrada: la suite automatizada cubre el backend (559 tests con *pytest* en 49 archivos) y, desde el 4 de septiembre de 2026, el módulo de alertas del frontend (49 tests con *Vitest*), pero no incluye todavía pruebas de extremo a extremo ni una métrica de cobertura de código, y la mayor parte de la interfaz sigue sin pruebas automatizadas; se declara como el objetivo de calidad más importante a resolver en la siguiente etapa del proyecto.
