...
Como respaldo transversal a varios de estos objetivos, la suite automatizada del backend alcanza 526 tests distribuidos en 46 archivos (ejecutados en integración continua), que cubren aislamiento por tenant, autenticación, rate limiting e importaciones, sumados a 25 pruebas de frontend sobre la lógica de alertas; no existen todavía pruebas de extremo a extremo (véase Pruebas).
...
- **Comunicación y campo:** `messages` y `conversation_sessions` (estado del asistente de WhatsApp), `bitacora_entries` (notas de voz con su transcripción, resumen y sugerencias), `historial` (registro de eventos *append-only*), `alerts` (con su nivel de severidad).
