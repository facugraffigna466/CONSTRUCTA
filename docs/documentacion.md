...
## 2026-09-10 — Dashboard Avanzado: implementación de los 13 indicadores (P0-P2)

### Objective
Llevar a código el diseño cerrado el día anterior (`docs/features/dashboard-indicadores-obra.md`). Se trabajó en tres etapas incrementales, cada una en su rama y su PR, siguiendo el plan de implementación que el propio documento dejó sugerido en su sección 10.

### Changes made

**Etapa P0** (`feature/dashboard-indicadores-p0`, PR #121, mergeado a `main`): los 5 indicadores imprescindibles. Módulo nuevo `dashboard_calc.py` con las fórmulas puras de §3 del diseño (peso de tarea, avance, avance planificado) y `working_days_between()` agregado a `calendar_service.py` — es el único helper de calendario que faltaba. `ObraDashboardService` compone todo en `GET /obras/{id}/dashboard`: avance real ponderado (I-01), avance planificado (I-02), SPI (I-03), fecha de fin proyectada (I-04), alertas por severidad (I-09) y cuello de botella (I-10, reusando `StaffDigestService`, que pasó a exponer un método público en vez de privado). D-02 del diseño —que el portfolio y el detalle de obra usen el mismo cálculo— se resolvió agregando `real_percent` a `ObraService.list_all` con el mismo módulo `dashboard_calc`. En el frontend, `ResumenTab` reemplazó la tira de KPIs por conteo simple por la banda nueva.

**Etapa P1** (`feature/dashboard-indicadores-p1`, PR #122): los 4 indicadores restantes que no necesitaban esquema nuevo. Línea base (I-06, desvío contra `task_baselines`), ruta crítica (I-07, agregados sobre el CPM que ya calculaba `TaskService`), hitos (I-08, estado por hito reusando el float del CPM) y materiales (I-11, ejecución vs. estimado con un indicador de alineación contra el avance real). Al construir I-07 apareció una inconsistencia real: el CPM corre sobre todas las tareas de la obra sin filtrar las canceladas, así que `critical + at_risk + slack` podía sumar más que el total usado por `coverage_percent` — se acotó al mismo universo. También se corrigió, en el mismo PR, un ajuste de UX señalado en revisión: el tile de Alertas mostraba "00" grande sin aclarar que era solo críticas, leyéndose como contradicción contra "1 alta · 4 media" debajo.

**Etapa P2** (`feature/dashboard-indicadores-p2`, PR #123): los dos indicadores que el diseño dejó explícitamente diferidos por necesitar esquema nuevo. Curva S (I-05): tabla `obra_progress_daily` (migración 0074) y un job diario en el scheduler (2:30 AM) que persiste, por cada obra activa, el mismo resultado que ya ve el usuario en el panel — se guarda el resultado calculado, no los insumos, siguiendo la decisión que el diseño ya había dejado tomada. Análisis del período (I-12/I-13): `GET /obras/{id}/dashboard/monthly-insights` expone la concentración de retraso 80/20 y la precisión de estimación por disciplina que `obra_stats_service.py` ya calculaba mensualmente pero que hasta ahora solo alimentaban el informe HTML interno (`insights.py`), sin ningún consumo desde la aplicación. D-01 (el ranking por responsable solo se muestra a administradores) se filtra en el backend: la clave `by_responsible` directamente no viaja en el JSON para un usuario no-admin, no se oculta en el frontend.

**Un bug real, encontrado en la verificación manual de P2, no en los tests.** La curva de avance no mostraba ninguna serie real pese a tener filas guardadas: el emparejamiento entre los puntos semanales del gráfico (anclados al día de la semana del inicio de la obra) y las filas de `obra_progress_daily` (ancladas al día en que el job arrancó a trackear) exigía fecha exacta, y esos dos anclajes casi nunca coinciden. Se cambió a la fila más cercana dentro de una tolerancia de tres días, con test de regresión. Es el mismo tipo de hallazgo que ya había aparecido en sesiones anteriores del proyecto: un bug que la suite automatizada no detecta porque los datos de prueba, sin querer, evitan el caso — solo aparece al mirar la pantalla con datos reales.

**Otro bug real, encontrado al escribir los tests de P1, no en la funcionalidad nueva en sí.** `_build_forecast` (parte de P0, ya en `main`) dividía por el SPI sin contemplar que puede dar exactamente `0.0` —una tarea sin ningún avance con el plan ya arrancado, un caso real y no un borde exótico— y tiraba `ZeroDivisionError`. Corregido tratando `spi <= 0` igual que "no confiable", con test de regresión.

### Files modified
Backend: `app/services/dashboard_calc.py`, `app/services/obra_dashboard_service.py`, `app/services/calendar_service.py`, `app/schemas/obra_dashboard.py`, `app/api/routes/obra_dashboard.py`, `app/core/scheduler.py`, `app/models/obra_progress_daily.py`, `app/services/staff_digest_service.py`, `app/services/obra_service.py` (todos nuevos o modificados a lo largo de las tres etapas); migración `alembic/versions/0074_obra_progress_daily.py`; tests nuevos `test_dashboard_calc.py`, `test_obra_dashboard.py`, `test_obra_progress_daily.py`, `test_curva_s.py`, `test_monthly_insights.py` (además de casos agregados a `test_calendar_working_days.py`). Frontend: `components/ResumenTab.tsx`, `components/CurvaSChart.tsx` y `components/MonthlyInsightsAccordion.tsx` (nuevos), `lib/dashboardMeta.ts`, `api/obraDashboard.ts`, `api/curvaS.ts`, `api/monthlyInsights.ts`, `types/index.ts`, `pages/PortfolioPage.tsx` (D-02), `pages/ObraDetailPage.tsx`. Documentación: esta entrada e `IPI-CONSTRUCTA.md` (RF-77 a RF-82, fila de trazabilidad del objetivo 8, módulo nuevo en Implementación, conteos de la suite y casos de prueba en Pruebas).
