...
| 6 | Sistema de alertas | Implementado, con una salvedad de aislamiento | Servicio de evaluación de riesgos, 6 tipos de alerta, emisión en tiempo real por Socket.IO, traza en el historial. | El endpoint administrativo de simulación de vencidos no filtra por tenant (ver Evolución del alcance); pendiente de corrección. |
...
| 9 | Historial y trazabilidad | Implementado como registro de aplicación | Tabla de historial con eventos de tareas, chatbot y alertas, escritos por los servicios. | No está reforzado a nivel de base de datos (sin triggers ni restricciones que impidan `UPDATE`/`DELETE`); es disciplina de código, no garantía de inmutabilidad. |
| 10 | Documentación técnica centralizada | Implementado | Repositorio de planos con versionado por obra y disciplina; consulta y envío de la última versión vigente por WhatsApp. | Ninguno relevante. |
...
| Plataformas de *construction management* (Procore, Autodesk Construction Cloud) | Cubren múltiples procesos de obra en un mismo ecosistema, incluida cierta captura de campo. | Implican costo y complejidad de implementación elevados, y exigen que todos los participantes adopten la plataforma completa. |
...
El análisis evita dividir el mercado entre "planificadores sin campo" y "herramientas de campo sin estructura", porque Procore y Autodesk ya conectan ambos entornos (Procore Technologies, s. f.; Autodesk, s. f.). Microsoft Project representa una referencia especialmente relevante por la profundidad de su planificación y por su presencia en el caso relevado. La carencia común a estas alternativas es la falta de un puente de baja fricción entre el plan y el campo: CONSTRUCTA se posiciona en ese espacio, tomando como canal de campo el mismo que el equipo ya utiliza.
...
- **RNF-01 (Seguridad):** la API autentica las operaciones con tokens JWT, aplica permisos por rol (administrador/colaborador) y contraseñas con *hashing*; los recursos sensibles usan URLs firmadas. La auditoría del 13/08/2026 identificó dos rutas sin filtro de tenant (presencia y simulación de vencidos administrativa; ver Evolución del alcance) que deben corregirse antes de afirmar un aislamiento estricto.
...
- **Auditoría técnica sistemática:** se auditó el sistema módulo por módulo —las 25 rutas del backend, los servicios y el modelo de datos— con ocho análisis (`docs/analisis-modulo-*.md`) consolidados en un informe maestro (`docs/auditoria-sistema-consolidada.md`) que clasifica los hallazgos por severidad (seguridad / robustez / pulido) e identifica como tema transversal principal el aislamiento entre empresas (*tenants*). Como resultado, la suite automatizada alcanza 72 tests distribuidos en 16 archivos, ejecutados en integración continua ante cada cambio, cubriendo aislamiento por tenant con *pytest* (`backend/tests/test_tenant_isolation.py`, que verifica que un usuario de una empresa no accede a los recursos de otra), además de autenticación, rate limiting, importaciones y robustez de infraestructura. Los tests de frontend y la automatización end-to-end quedan pendientes; se declaran explícitamente en las secciones de Implementación y de Objetivos no cumplidos.
...
> **Estado de la cobertura automatizada.** El backend cuenta con 72 tests automatizados (*pytest*, 16 archivos) corriendo en integración continua. No existen todavía pruebas automatizadas de frontend (*Vitest* u otro) ni de extremo a extremo (*Playwright*); se deja declarada esta brecha como parte de los objetivos pendientes del proyecto.
...
- Cámara Argentina de la Construcción. (2026). *Innovación, tecnología y sustentabilidad*. https://www.camarco.org.ar/newsletter-news/innovacion-tecnologia-sustentabilidad/
