...
- **Auditoría técnica sistemática:** se auditó el sistema módulo por módulo —las 26 rutas del backend, los servicios y el modelo de datos— con ocho análisis (`docs/analisis-modulo-*.md`) consolidados en un informe maestro (`docs/auditoria-sistema-consolidada.md`) que clasifica los hallazgos por severidad (seguridad / robustez / pulido) e identifica como tema transversal principal el aislamiento entre empresas (*tenants*). Como primer arnés de pruebas automatizadas se incorporó un conjunto de tests de aislamiento multi-inquilino con *pytest* (`backend/tests/test_tenant_isolation.py`), que verifica que un usuario de una empresa no accede a los recursos de otra.
